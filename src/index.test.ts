...
TURSO_AUTH_TOKEN="1234"`
		...
		mockedExistsSync.mockImplementation((path) => {
			return path === '.env.example' || path === '.env'
		})

		mockedReadFileSync.mockImplementation((path) => {
			if (path === '.env.example') return envExampleContent
...
TURSO_AUTH_TOKEN="1234"`
		...
		expect(mockedWriteFileSync).toHaveBeenCalledWith('.env', expectedOutput)
	})
...
PAYLOAD_SECRET="abc123"`
	...
	it('should handle custom file paths', async () => {
		...
		mockedExistsSync.mockImplementation((path) => {
			return path === '.env.custom.example' || path === '.env.custom'
		})

		mockedReadFileSync.mockImplementation((path) => {
			if (path === '.env.custom.example') return envExampleContent
			if (path === '.env.custom') return envContent
			throw new Error('File not found')
		})
		...
		expect(mockedWriteFileSync).toHaveBeenCalledWith('.env.custom', 'KEY="different"')
	})
