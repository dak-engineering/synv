...
const isValidEnvironmentVariableLine = (line) => {
...
const getFileContentsByLine = async ({ relativeFilePath, files, defaultFileName, createIfNotExists = false, }) => {
...
const loadWhile = async (message, cb, { successMessage, failureMessage, throwOnError = true, } = {}) => {
...
const getFileLinesInArray = async (filepath) => {
...
const promptForFilePath = async (files, message) => {
    return search({
        message,
        source: async (input) => {
            if (!input) {
...
const highlightFuzzyMatches = (files, input) => {
...
const autoDetectFilePath = async (files, filenameToDetect) => {
